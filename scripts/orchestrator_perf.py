...
def analyze_performance(input_text: str, num_runs: int = 100):
    ...
    for _ in range(num_runs):
        ...
        layer_b_times.append(result.layer_b_time_ms or 0.0)
        layer_c_times.append(result.layer_c_time_ms or 0.0)
    ...
    return {
        ...
        'layer_c_time_ms': {
            'avg': sum(layer_c_times) / num_runs,
            'min': min(layer_c_times),
            'max': max(layer_c_times)
        }
    }